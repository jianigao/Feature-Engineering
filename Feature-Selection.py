# Filter-based method: Pearson Correlation

# We check the absolute value of the Pearson’s correlation between the target and
# numerical features in our dataset. We keep the top n features based on this criterion.
...
# Filter-based method: Chi-square

# We calculate the chi-square metric between the target and the numerical 
# variable and only select the variable with the maximum chi-square values.

from sklearn.feature_selection import SelectKBest
from sklearn.feature_selection import chi2
from sklearn.preprocessing import MinMaxScaler

X norm = MinMaxScaler().fit_transform(X)
...
'''
We select features by recursively considering smaller and smaller sets of features.
First, the estimator is trained on the initial set of features and the importance of 
each feature is obtained either through a coef_ attribute or through a feature_importances_ 
attribute. Then, the least important features are pruned from current set of features. 
That procedure is recursively repeated on the pruned set until the desired number of 
...
feature_selection_df = pd.DataFrame({'Feature':feature_name, 'Pearson':cor_support, 'Chi-2':chi_support,
	'RFE':rfe_support, 'Logistics':embeded_lr_support, 'Random Forest':embeded_rf_support, 'LightGBM':embeded_lgb_support})
